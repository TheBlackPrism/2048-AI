...
def find_best_move(board):
    ...
    if biggestTile < 4000:
        scorefunction = snakescore
    else:
        scorefunction = diagscore

    start = time.time()
    if emptyTiles > 8 or biggestTile < 1024:
        result = [score_toplevel_move(x, board, scorefunction, 1) for x in range(len(move_args))]
    elif emptyTiles > 3 or biggestTile < 2048:
        result = []
        r = []  
        for x in range(len(move_args)):
            r.append(p.apply_async(score_toplevel_move, (x, board, scorefunction, 2)))
        ...
        r = []  
        for x in range(len(move_args)):
            r.append(p.apply_async(score_toplevel_move, (x, board, scorefunction, 3)))
        for i in range(len(r)):
            result.append(r[i].get())
...
def score_toplevel_move(move, board, scorefunction = snakescore, depth=2):
    """
    ...
    if depth == 0:
        return getScore(newboard, scorefunction)
    ...
    for i in range(4):
        for j in range(4):
            ...
            cntEmptyFields += 1
            newboard[i,j] = 2
            maxscore = max([score_toplevel_move(m, np.copy(newboard), scorefunction, depth) for m in range(len(move_args))])
            score += 0.9 * maxscore
...
def getScore(board, scorefunction):
    return np.sum(np.multiply(board, scorefunction))
